...
export function getDirection8(vector: Vector2): Point | null {
    ...
    if (Math.abs(angle) < Math.PI * 0.125) return new Point(1, 0);
    if (Math.abs(angle) > Math.PI * 0.875) return new Point(-1, 0);
    if (angle < 0) {
        if (Math.abs(angle) < Math.PI * 0.375) return new Point(1, -1);
        if (Math.abs(angle) > Math.PI * 0.625) return new Point(-1, -1);
...
export function getDirection4(vector: Vector2): Point | null {
    ...
    if (Math.abs(angle) < Math.PI * 0.25) return new Point(1, 0);
    if (Math.abs(angle) > Math.PI * 0.75) return new Point(-1, 0);
